...
static const uint8_t NB_HORIZONTAL_MATRIX = 4; // MUST be 4 (= 128px per line)
...
int main(void) {
  ...
  for (;;) {
    ...
    // For each lines
	for (unsigned int y = 0; y < img_Lenna128_height; /* Incremented by the scrolling animation */) {
	
	  /* Get the first line byte address */
	  uint16_t ptr = pgm_read_word(&img_Lenna128_data[y]);
	
	  // For each pixels
	  for (unsigned int x = 0; x < NB_COLUMNS_COUNT; ++x) {

	    /* Get the first byte address */
		uint16_t _ptr = ptr + x * 3;
	  
	    /* Get the RGB data */
		uint8_t r = pgm_read_byte(_ptr);
		uint8_t g = pgm_read_byte(++_ptr);
		uint8_t b = pgm_read_byte(++_ptr);
		
		/* Set the pixel */
		setPixelAt(x, scrollCount, r, g, b);
	  }
	  ...
	  // Scrolling animation
	  if (++scrollCount == NB_LINES_COUNT) {
		
		// Reset scroll counter
		scrollCount = 0;
		
		// Shift frame by one line
		y -= NB_LINES_COUNT - 1;
		
	    // Inter animation frame delay
        _delay_ms(25);
	  }
	}
  }
